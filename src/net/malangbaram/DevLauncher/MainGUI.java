...
/*
 *  TeamCroatia Dev-Launcher Main Class
 *  프로그램의 메인클래스
 *
 * 	@author MalangBaram
 */
...
public class MainGUI {
	...
	public static void main(String[] args) throws Exception {
		...
		if (MainGUI.lastVersion.equals(MainGUI.myVersion))
			updatePlease = false;
		else
			updatePlease = true;
	...
	private static void mainForm() {
		...
		JButton Dbtn = new JButton("다운로드");
		container.add(Dbtn);
		JButton Lbtn = new JButton("런처 실행");
		container.add(Lbtn);

		final JLabel stat = new JLabel("다운로드 준비중");
		container.add(stat);

		if (MainGUI.updatePlease) {
			JOptionPane.showConfirmDialog(null, "최신버전이 존재합니다. 모드팩을 업데이트 해 주세요.", Lang.TITLE, JOptionPane.DEFAULT_OPTION,
		...
		Dbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showConfirmDialog(null, "해당 설치기 실행시 모두 초기화되며 현재 백업기능은 지원하지 않으니 백업 후 설치하시길 권장합니다.",
						Lang.TITLE, JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);

				JOptionPane.showConfirmDialog(null, "완료창이 뜰 때 까지 프로그램을 종료하지 말아주세요.", Lang.TITLE,
				...
				try {
					...
					JOptionPane.showConfirmDialog(null, "다운로드 오류", Lang.TITLE, JOptionPane.DEFAULT_OPTION,
							JOptionPane.INFORMATION_MESSAGE);
		...
		Lbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (MainGUI.myVersion.equals("미설치")) {
					JOptionPane.showConfirmDialog(null, "현재 로컬에 모드팩이 설치되어 있지 않습니다. 설치 후 다시 시도 해주세요.", Lang.TITLE,
							JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
				} else {
